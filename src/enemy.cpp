...
Viserys::Viserys(int scene) {
    ...
    this->position = glm::vec3(x, y, 0);
    this->rotation = 0;
    this->part = Polygon(this->position.x,this->position.y,COLOR_DEAD_BLACK,1.0f,6,0);    
}

void Viserys::draw(glm::mat4 VP) {

    for(std::vector<Polygon>::iterator it = bullets.begin(); it != bullets.end(); ++it) {
    it->draw(VP);
    }
    this->part.draw(VP);    
}
...
void Viserys::tick() {
    Polygon p = Polygon(this->position.y,this->position.x,COLOR_INDIGO,0.2f,10,0);
    bullets.push_back(p);
for(std::vector<Polygon>::iterator it = bullets.begin(); it != bullets.end(); ++it) {
    it->position.x -= 0.05f;
